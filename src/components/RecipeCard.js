const RecipeCard = ({image, title, description, time}) => {
  return (
    <div className="relative z-10 h-[305px] md:w-[225px] w-[250px] flex-col flex justify-end">
      ...
      <h2 className="font-ubuntu text-lg">{title}</h2>
      <p className="font-ubuntu opacity-50 text-base">{description}</p>
      <button className="font-pangolin bg-secondary-red bg-black text-white px-12 py-3 mt-2 rounded-lg">Ver</button>
      </div>
    </div>
  )
}
